...
exe_path=exe_path.gsub("CoursePowerPlus.exe","")
chromedriver_path=exe_path.dup.to_s.gsub("CoursePowerPlus.exe","chromedriver.exe")
...
file_inside_current_dir=[]
  ...
  if file_inside_current_dir.include?("settings.env")
    ...
    path_for_download=path_for_download.encode("UTF-8")#pathに日本語が含むとき、powershellで実行するとshift-jsで入力されてしまうので変換。
#環境設定ファイルの新規作成と書き込み
...
file_inside_current_dir=[]
   ...
   if file_inside_current_dir.include?("settings.env")
    ...
    path_for_download=path_for_download.gsub("\\","\\\\\\")
    path_for_download=path_for_download.encode("UTF-8")
    File.open("settings.env","w") do |file|
    file.puts("USER_ID=\"#{user_id}\"")
    file.puts("PASSWORD=\"#{password}\"")
    file.puts("PATH_FOR_DOWNLOAD=\"#{path_for_download}\"")
